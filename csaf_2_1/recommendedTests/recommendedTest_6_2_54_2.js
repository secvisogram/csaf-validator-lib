...
/**
 * This implements the recommended test 6.2.54.2 of the CSAF 2.1 standard.
 *
 * @param {unknown} doc
 */
export async function recommendedTest_6_2_54_2(doc) {
  const ctx = {
    warnings:
  ...
  for (const path of X_EXTENSIONS_PATHS) {
    await walkPath(doc, path, async (instancePath, value) => {
      ...
      if (classifyExtensionSchema(schemaUrl) !== 'official') {
        ctx.warnings.push({
          instancePath: `${instancePath}`,
          message: 'the extension is not an official CSAF Extension',
        })
      }
    })
  }
