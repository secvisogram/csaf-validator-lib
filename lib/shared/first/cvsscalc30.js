...
/**
 * Build a result object from a successfully parsed CVSS30 instance.
 *
 * @param {CVSS30} cvss
...
function buildResult(cvss, vectorString) {
  ...
  return {
    ...
    baseSeverity: CVSS30.Rating(baseScore),

    temporalMetricScore: temporalScore.toFixed(1),
    temporalSeverity: CVSS30.Rating(temporalScore),

    environmentalMetricScore: environmentalScore.toFixed(1),
    environmentalSeverity: CVSS30.Rating(environmentalScore),
...
/**
 * Calculate CVSS 3.0 scores from a vector string.
 *
...
/**
 * Calculate CVSS 3.0 scores from individual metric parameters.
 *
 * Assembles a vector string from the 22 positional parameters (using the same
 * metric ordering defined by the CVSS v3.0 specification) and delegates to
