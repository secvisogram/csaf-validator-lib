...
// Pattern for match all
export const urlPatternGlobal = new RegExp(urlPattern, 'g')
...
export default async function informativeTest_6_3_8(
  ...
  function segmentString(text) {
    if (text) {
      // collect URL's to check them as whole
      const urlStringMatches = text.matchAll(urlPatternGlobal)
      // remove the URL's from the text
      const textWithOutUrl = text.replace(urlPatternGlobal, '')
      // segment text
      const segmentedText = segmenter.segment(textWithOutUrl)
      const segments = [...segmentedText]
        .filter((s) => s.isWordLike)
        .map((s) => s.segment)
      // add collected URL's to the segmented text to check them as a whole
